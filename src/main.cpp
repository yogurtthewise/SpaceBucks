...
// Copyright (c) 2013 The SpaceBucks developers
...
uint256 hashGenesisBlock("0x00000086e1ee47b4ba8fed6d2c7f3f309d121795af9e4b69657da9ba34d801c7");
static const unsigned int timeGenesisBlock = 1402255999;
...
const string strMessageMagic = "SpaceBucks Signed Message:\n";
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "We're not doing it for the money; We're doing it for a SHITLOAD of money!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 29341661;

        if (fTestNet)
        {
            block.nTime    = 1402255999;
            block.nNonce   = 29341661;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x35539de0d84cd6f76cbedcbf43705e638f4bebff5d429d112527456e87f5e9fa"));
...
unsigned char pchMessageStart[4] = { 0xbe, 0xbe, 0xbe, 0xbe };
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = Hashblake(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = Hashblake(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = Hashblake(vRecv.begin(), vRecv.begin() + nMessageSize);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpaceBucksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpaceBucksMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("SpaceBucksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spacebucks-miner");
    ...
    try { loop {
        ...
        printf("Running SpaceBucksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("SpaceBucksMiner terminated\n");
        throw;
    }
}
